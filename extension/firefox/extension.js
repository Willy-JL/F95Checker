...
const reportRpcError = (tabId) => {
    if (tabId) {
        chrome.scripting.executeScript({
            target: { tabId: tabId },
            func: () => {
                alert('Could not connect to F95CheckerX!\nIs it open and updated? Is RPC enabled?');
            },
        });
    }
};
...
const addGame = async (url, tabId) => {
    await rpcCall('POST', '/games/add', JSON.stringify([url]), tabId).then(() => render(tabId));
};

const addReminder = async (url, tabId) => {
    await rpcCall('POST', '/reminders/add', JSON.stringify([url]), tabId).then(() => render(tabId));
};

const addFavorite = async (url, tabId) => {
    await rpcCall('POST', '/favorites/add', JSON.stringify([url]), tabId).then(() => render(tabId));
};
        ...
        chrome.scripting.executeScript({
            ...
            func: (settings, games, tags) => {
                ...
                const removeOldIcons = () => {
                    document
                        .querySelectorAll('.f95checkerx-library-icons')
                        .forEach((e) => e.remove());
                };
                const addHrefIcons = () => {
