        ...
        def custom_context_menu_requested(pos: QtCore.QPoint):
            ...
            if (url := data.linkUrl().url()):
                if "f95zone.to/threads/" in url:
                    add_game = QtGui.QAction(icon, "Add this link to F95Checker", menu)
                    add_game.triggered.connect(lambda _: app.window.webview.page.runJavaScript(f"addGame({url!r});"))
                    menu.addAction(add_game)
                    add_reminder = QtGui.QAction(icon, "Add this link to reminders", menu)
                    add_reminder.triggered.connect(lambda _: app.window.webview.page.runJavaScript(f"addReminder({url!r});"))
                    menu.addAction(add_reminder)
            elif "f95zone.to/threads/" in (url := app.window.webview.url().url()):
                add_game = QtGui.QAction(icon, "Add this page to F95Checker", menu)
                add_game.triggered.connect(lambda _: app.window.webview.page.runJavaScript(f"addGame({url!r});"))
                menu.addAction(add_game)
                add_reminder = QtGui.QAction(icon, "Add this page to reminders", menu)
                add_reminder.triggered.connect(lambda _: app.window.webview.page.runJavaScript(f"addReminder({url!r});"))
                menu.addAction(add_reminder)
